...
export default function initMixin(Table) {
  Table.prototype._init = function (options = {}) {
    ...
    const colgroup = querySelector(table, "colgroup")
    if (!table) return console.info("Smart Table init need a colgroup")
    ...
    const tfoot = querySelector(table, "tfoot")
    if (!tfoot) return console.error("Smart Table init need a tfoot")
    ...
    //初始化fixed元素及宽度
    initFixed(vm)
    createFixed(vm, thead, tbody, tfoot)
    // }
    ...
    setAttribute(td, "width", vm.gutterWidth);
    setAttribute(th, "rowspan", vm.props.shapes.length);
    setAttribute(td, "rowspan", vm.props.shapes.length);
    appendChild(querySelector(vm.$thead, "tr"), th);
    if (vm.$tfoot.children.length > 0) {
      appendChild(querySelector(vm.$tfoot, "tr"), td);
    }
    vm.$scrollTH = th;
    if (vm.scrollY) {
      styled(vm.$scrollTH, {
...
function appendExpandTrigger(vm) {
  ...
  let tfootRow = querySelector($tfoot, "tr");
  let tfootTd = tfootRow.querySelector("td");
  let tfootCell = createExpandTrigger("td", getAttrNumber(th, "rowspan", 1));
  insertBefore(tfootRow, tfootCell, tfootTd)
}
...
function layout(vm, table) {
  ...
  querySelectorAll($tfoot, "td").forEach(cell => refactorCell(cell))
  ...
  vm.$tfootWrapper = createTableWrapper("stb_footer-wrapper", vm, "footer", $tfoot);
  appendChildren($root, [vm.$theadWrapper, vm.$tbodyWrapper, vm.$tfootWrapper]);
...
function initColgroupData(vm) {
  ...
  let arr = [];
  if (vm.$colgroup) {
    for (var i = 0; i < vm.$colgroup.children.length; i++) {
      let width = parseInt(vm.$colgroup.children[i].width)
      arr.push(width);
    }
  } else {
...
function bindEvents(vm) {
  ...
  vm.options.expand ? bindExpandFixedTreeEvents(vm) : bindSortEvents(vm);
  bindResizeEvents(vm);
}
  ...
  debounce(500, () => {
    ...
    if (vm.$fixedLeft && fixedLeft.thead.length) {
      ...
      styled(vm.$fixedLeft, {
        ...
        height: height + (vm.$tfoot.children.length ? 8 : 0) + "px"
      })
    }
...
function syncPostion(vm) {
  throttle(20, () => {
    ...
    vm.$tfootWrapper.scrollLeft = vm.$tbodyWrapper.scrollLeft;
    if (vm.$fixedLeftBody) {
...
function handleFixedMousewheel(event, vm) {
  const bodyWrapper = vm.$tbodyWrapper;
  if (Math.abs(event.deltaY) > 0) {
    const currentScrollTop = bodyWrapper.scrollTop;
    if (event.deltaY < 0 && currentScrollTop !== 0) {
      event.preventDefault();
    }
    if (event.deltaY > 0 && bodyWrapper.scrollHeight - bodyWrapper.clientHeight > currentScrollTop) {
      event.preventDefault();
    }
    bodyWrapper.scrollTop += Math.ceil(event.deltaY / 5);
  } else {
    bodyWrapper.scrollLeft += Math.ceil(event.deltaY / 5);
  }
}

function bindExpandFixedTreeEvents(vm) {
  function seek(arr) {
    arr.forEach(item => {
      if (item.expand) {
    ...
    function expanding(item, expanded) {
      vm.expandData.forEach(i => {
        if (JSON.stringify(i) === JSON.stringify(item)) {
          expanded ? setAttribute(i.$el, "expanded") : removeAttribute(i.$el, "expanded");
          i.children && (i.children.forEach(child => {
            styled(child.$el, {
...
function bindScrollEvents(vm) {
  ...
  if (vm.$fixedLeftBody) {
    vm.$fixedLeftBody.addEventListener("mousewheel", (e) => handleFixedMousewheel(e, vm))
  }
  if (vm.$fixedRightBody) {
    vm.$fixedRightBody.addEventListener("mousewheel", (e) => handleFixedMousewheel(e, vm))
  }
}
